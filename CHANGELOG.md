# Changelog

## [2.1.3] - 2026-01-13

### Added

- Added workflow to build zip without version number

## [2.1.1] - 2026-01-13

### Fixed

- Added PUC Library

## [2.1.0] - 2026-01-13

### Added
- Automatic updates via GitHub releases
- Plugin Update Checker library integration
- Version constants for better version management
- Input validation for GTM Container ID format
- Admin notice when GTM ID is not configured
- Dismissible admin notices with AJAX handler
- Activation, deactivation, and uninstall hooks
- Enhanced settings page with version info
- Text domain for internationalization support
- Support for Avada and Salient themes
- Fallback to standard WordPress hooks

### Features
- Automatic GTM head script injection
- Automatic GTM body noscript injection
- Theme-aware hook selection
- Clean uninstall (removes all data)
